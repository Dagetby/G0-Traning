...
func (store BookmarkStore) Update(b Bookmark) error {
	err := store.C.Update(bson.M{"_id": b.ID}, bson.M{"$set": bson.M{
		"name":        b.Name,
...
func (store BookmarkStore) GetByTag(tag []string) []Bookmark {
	var b []Bookmark
	iter := store.C.Find(bson.M{"tags": bson.M{"$in": tag}}).Sort("priority",
		"-createdon").Iter()
	result := Bookmark{}
	for iter.Next(&result) {
		b = append(b, result)
